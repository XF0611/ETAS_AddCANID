...
#define DEM_MAP_ORIGIN2ID_OBD_RELEVANT      DEM_MAP_ORIGIN2ID_INVALIDORIGIN
...
#define DEM_MAP_ORIGIN2ID  {                                        \
        ...
        DEM_MAP_ORIGIN2ID_OBD_RELEVANT,      \
        DEM_MAP_ORIGIN2ID_SECONDARY         \
}

#if (DEM_DTC_ORIGIN_PRIMARY_MEMORY   != 0x0001) \
        || (DEM_DTC_ORIGIN_MIRROR_MEMORY    != 0x0002) \
        || (DEM_DTC_ORIGIN_PERMANENT_MEMORY != 0x0003) \
        || (DEM_DTC_ORIGIN_OBD_RELEVANT_MEMORY != 0x0004)\
        || (DEM_DTC_ORIGIN_SECONDARY_MEMORY != 0x0005)
...
#include "Dem_MemMap.h"
/* MR12 RULE 20.7 VIOLATION: Functions like MACROS do not need to be parenthesized.*/
DEM_ARRAY_DEFINE_CONST(Dem_EvMemMapOrigin2IdType, Dem_EvMemMapOrigin2Id, 6, DEM_MAP_ORIGIN2ID);
#define DEM_STOP_SEC_CONST
#include "Dem_MemMap.h"
...
static void Dem_EvMemSetStatusNotification(uint16_least LocId, Dem_EventIdType EventId, Dem_EvMemActionType actionType)
{
    ...
    if (Dem_EvMemIsStatusNotificationRequired(LocId))
    {
        if (actionType!=DEM_EVMEM_ACTIONTYPE_NONE)
        {
            ...
            {
/* BSW-11069 */ /* [$DD_BSWCODE 35100] */
#if (DEM_CFG_SETTING_PENDINGDTC_INDEPENDENT_FROM_STOCO == TRUE)
                /* Do nothing, PendingDTC bit is set by Dem_EvtSt_HandleFailed */
#else
/* END BSW-11069 */
...
/* BSW-12998 */ /* [$DD_BSWCODE 34697] */
#if (DEM_CFG_OBD != DEM_CFG_OBD_OFF)
#if (DEM_CFG_OBD_CONSIDER_MILCNT == DEM_CFG_OBD_CONSIDER_MILCNT_ON)
    uint16_least       lowestPrio = 0;
    boolean            isEmissionRelatedEv = FALSE;
    uint16_least       LowestPrioSearchLocId = DEM_EVMEM_INVALID_LOCID;
    uint16_least       SearchTimeId_LowestPrio = 0;
/* END BSW-12998 */
#endif
#endif
    uint16_least       Active;
...
/* BSW-12998 */ /* [$DD_BSWCODE 34697] */
#if (DEM_CFG_OBD_CONSIDER_MILCNT == DEM_CFG_OBD_CONSIDER_MILCNT_ON)
    isEmissionRelatedEv = rba_DemObdBasic_EvMem_IsEmissionRelated(EventId);
#endif
/* END BSW-12998 */
    ...
    {    /* only stored memory entries needs to be considered */
        ...
        {   /* Displace only if allowed */
            ...
            )
            {
                ...
                {
                   /* If both events have the same priority, displacement only happens if event (stored in EvMem) is not tested in this OC */
...
#if (DEM_CFG_OBD != DEM_CFG_OBD_OFF)
#if (DEM_CFG_OBD_CONSIDER_MILCNT == DEM_CFG_OBD_CONSIDER_MILCNT_ON)
                if (TRUE == isEmissionRelatedEv)
                {   
                    /* When no longer MIL, seeking for the lowest priority & active DTC & oldest timeId  */
                    if ((Prio > lowestPrio) || ((Prio == lowestPrio)&& ( TimeId <= SearchTimeId_LowestPrio)))
                    {
                        LowestPrioSearchLocId = LocId;
                        lowestPrio = Prio;
                        SearchTimeId_LowestPrio = TimeId;
                    }
                }
#endif
#endif
...
#if (DEM_CFG_OBD != DEM_CFG_OBD_OFF)
#if (DEM_CFG_OBD_CONSIDER_MILCNT == DEM_CFG_OBD_CONSIDER_MILCNT_ON)
    else if (LowestPrioSearchLocId != DEM_EVMEM_INVALID_LOCID)
    {
        Dem_EvMemForceClearEventMemoryLocation(LowestPrioSearchLocId, DEM_EVMEM_WRITEMASK_CLEAR, DEM_EVMEM_ACTIONTYPE_DISPLACEMENT);
        SearchLocId = LowestPrioSearchLocId;
    }
#endif 
#endif 
            ...
            if (Dem_EvMemIsEventMemLocIdValid (LocId))
            {
               ...
               if (Dem_EvMemIsEdgeTrigger (StatusOld, StatusNew, DEM_EVMEM_STSMASK_TESTFAILED_TFC))
               {
                ...
                if (rba_DemObdBasic_EvMem_IsTriggerObdFreezeFrame(EventId, MemId, StatusOld, StatusNew))
                {
                    ...
                    rba_DemObdBasic_FF_SetObdFFAvailable(LocId, TRUE);
                }
#endif
...
/* BSW-12998 */ /* [$DD_BSWCODE 40512] */
#if ((DEM_CFG_OBD != DEM_CFG_OBD_OFF) && (DEM_CFG_OBD_CONSIDER_MILCNT == DEM_CFG_OBD_CONSIDER_MILCNT_ON))
                    /* Initialized value of healedId every time new event is set */
                    rba_DemObdBasic_EvMem_SetFFHealedId(LocId, 0);
#endif
/* END BSW-12998 */
                    /* set new time stamp  */
...
void Dem_EvMemSetNextReportRelevantForStorageFilteredEventsForEvCombOnStorage(Dem_EventIdType EventId)
{
    ...
    if (Dem_DtcIsSupported(dtcId))
    {
                ...
                Dem_EventIdListIteratorNext(&eventIt))
        {
            ...
            {
                Dem_EvtSetNextReportRelevantForMemories(eventId, TRUE);
            }
        }
    }
}
...
void Dem_EvMemSetEventPassed(Dem_EventIdType EventId, uint16_least MemId, const uint8 *EnvData)
{
    ...
    if(!Dem_GetEvMemLockInternal())
    {
        ...
        if (Dem_EvMemIsEventMemLocIdValid (LocId))
        {
            ...
            {
                /* set next report relevant for all other events on the same dtc that are storage filtered */
                Dem_EvMemSetNextReportRelevantForStorageFilteredEventsForEvCombOnStorage(EventId);
            }
#endif
		}
	}
...
#if (DEM_CFG_OBD != DEM_CFG_OBD_OFF)
            //special handling for OBD cases, TFTAC has to be reset in conjunction with the MIL
            if(Dem_EvMemEventMemory[LocId].ObdMilResetThisCycle == TRUE)
            {
                            ...
                            ( ! Dem_LibGetParamBool(DEM_CFG_SUPPORT_AGINGCNTR_ONLY_WHEN_AGING_ALLOWED) || Dem_EvtParam_GetAgingAllowed(EventId) ) )
                    {
                        ...
                        if (!IsCleared)
                        {
                            ...
                            if(Dem_EvtParam_GetAgingAllowed (EventId))
                            {
                                ...
                                {
                                    /* MR12 RULE 10.8 VIOLATION: No prblem seen in assignment. uint16 value is assigned to uint16 variable.   */
...
#if(DEM_CFG_DATAELEMENT_LASTFAILEDSINCELASTFAILEDEXCLUDINGTNC_INCLUDESFAILEDCYCLE == FALSE)
...
Dem_NvmBlockIdType Dem_EvMemGetNvmIdFromLocId(uint16_least LocId)
{
    ...
    {
        DEM_DET(DEM_DET_APIID_EVMEMGETNVMIDFROMLOCID,0,0);
    }
...
{
    Dem_EvMemActionType actiontype = DEM_EVMEM_ACTIONTYPE_NONE;
    uint16_least WriteSts = DEM_EVMEM_WRITEMASK_NO | DEM_EVMEM_WRITEMASK_DATA;
    ...
    Status = Status & (uint16_least)(~((uint16_least)DEM_EVMEM_STSMASK_TESTCOMPLETE_TFC));
    Status = Status & (uint16_least)(~((uint16_least)DEM_EVMEM_STSMASK_TESTCOMPLETE_TAC));

    if ( ! Dem_LibGetParamBool(DEM_CFG_SUPPORT_AGINGCNTR_ONLY_WHEN_AGING_ALLOWED) || Dem_EvtParam_GetAgingAllowed(EventId) )
    {
        Status = Status & (uint16_least)(~((uint16_least)DEM_EVMEM_STSMASK_AGED));

        Dem_EvMemSetEventMemAgingCounter(EvMemLocId,0);
#if (DEM_CFG_TFSLC_RESET_AFTER_AGING_AND_DISPLACEMENT)
        Dem_EvMemSetEventMemAgingCounterForTFSLC(EvMemLocId, 0);
#endif
    }
