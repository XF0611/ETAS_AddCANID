...
Xcp_ErrorCode XcpAppl_SetCalPage(uint8 Mode, uint8 SegNum, uint8 PageNum, uint8 ProtocolLayerId)
{
  ...
  if(PageNum >= (uint8)NUM_OF_CAL_PAGE)
  {
	  Error = XCP_ERR_PAGE_NOT_VALID;
  }
  else if (PageNum == (uint8)CalPage_Working)

  {
      Overlay_Enable();
      XcpCurrentCalPage = CalPage_Working;
  }
  else /* CalPage_Reference */
  {
      Overlay_Disable();
      XcpCurrentCalPage = CalPage_Reference;
  }
