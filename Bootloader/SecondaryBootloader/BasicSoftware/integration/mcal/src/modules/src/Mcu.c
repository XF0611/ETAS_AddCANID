...
Mcu_PeripheralClockValType Mcu_GetPeripheralClkFreq(Mcu_PeriphNameType PeripheralName)
{
    ...
    /**
     * @brief ETAS BIP Porting: McuClockInit shall not be checked for Mcu module when run in SBL mode.
     * As Mcu is initialized in PBL rather than in SBL.
     * Otherwise the failed check will lead to following SBL Fls-Spi initializaiton fail due to uninitialized
     * variables.
