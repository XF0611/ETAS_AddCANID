...
FUNC(void, OS_CALLOUT_CODE) StartupHook(void) 
{
    ...
    * */
    ...
    //ETAS BIP Porting TODO Rtm_Init();
#if (OS_NUM_CORES > 1)
    //ETAS BIP Porting TODO RTM_CORECHECKPOINT_CORE_OSSTART_SYNCH();
#else
    //ETAS BIP Porting TODO RTM_CORECHECKPOINT_CORE_OSSTART();
#endif
...
#include "ImageM_StreamRam.h"
uint32 tyFlg = 0;
uint32 tyI = 0;
uint32 tyAddr;
uint32 tyLength;
uint8 tyRead[4096]={"tanyang"};
extern ImageM_StreamFlashType pvBlockStream;
FUNC(boolean, OS_CALLOUT_CODE) Os_Cbk_Idle(void)
{
    ...
    if(tyFlg == 2)
    {
        ...
        {
            RetValue = ImageM_InspectEraseImageImpl(tyAddr, tyLength, &Action, &ErrCode);
            if(RetValue == E_OK)
            {
                ...
                if(ImageM_InitImageBlock(pImgContext, Action->FaId, Action->Offset, Action->Len) == E_OK)
                {
                    ...
                    {   
                        ...
                        ImageM_ImageStatus_SynchWriteApplicationValidFlag(pImgContext->isValid,pImgContext->imageId);
                        RetValue = E_OK;
                        tyAddr += 0x1000;
                    }
                    else
            ...
            {
                if(*ErrCode == 0x20u /*IMAGEM_E_REQ_REGION_INVALID*/)
                {
                    while(1);
                }
            }
    ...
    else if(tyFlg == 3)
    {
        ...
        ImageM_StreamFlash_write(&pvBlockStream, &tyRead[0], tyLength, FALSE);
